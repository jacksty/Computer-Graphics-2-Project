...
def bvh2rig(bvhfile,rigfile,ascii=True,debug=False):
    ...
    padwrite("heads",len(bonelist)*4*4,"bytes (one x,y,z,parent_index quadruple per bone; all floats)")
    ...
    for b in bonelist:
        ...
        if p == None:
            ...
            pidx = namemap[p.name]
        if ascii:
            write(b.head.x,b.head.y,b.head.z,pidx)
        else:
            fp.write(struct.pack("ffff",b.head.x,b.head.y,b.head.z,pidx))
    
    padwrite("tails",len(bonelist)*4*4,"bytes (one x,y,z,c quadruple per bone; c=1 if bone has end effector; 0 otherwise)")
    
    #write tails of bones.
    for b in bonelist:
        if b.endeff != None:
            if ascii:
                write( b.endeff.x,b.endeff.y,b.endeff.z,1.0 )
            else:
            ...
            if ascii:
                write(b.ch[0].head.x,b.ch[0].head.y,b.ch[0].head.z,0.0)
            else:
                fp.write( struct.pack("ffff",b.ch[0].head.x,b.ch[0].head.y,b.ch[0].head.z,0.0))
          
    padwrite("quaternions",numframes*len(bonelist)*4*4,
        "bytes (for each frame: four floats per bone: numframes*numbones*4*sizeof(float) bytes total)")
    ...
    padwrite("matrices",numframes*len(bonelist)*4*16,
        "bytes (for each frame: 16 floats per bone: numframes*numbones*16*sizeof(float) bytes total. Note: Written "+
        "in COLUMN MAJOR order; includes translation by headpos)")
    ...
    padwrite("translations",numframes*len(bonelist)*4*4,"bytes (for each frame: 4 floats per bone)")
    
    #frame-by-frame data for translations
    for i in range(numframes):
        for b in bonelist:
            if ascii:
                write(b.tframedata[i][0],b.tframedata[i][1],b.tframedata[i][2],0)
            else:
    ...
    print("Done. Depth=",maxdepth,"; numbones=",len(bonelist),"; numframes=",numframes,
    ...
    #print("Data for frame 3:")
    #for b in bonelist:
    #    print(b.idx,b.name,b.framedata[3],b.qframedata[3],b.tframedata[3])
